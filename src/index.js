...
function lineBuffer (line, radius, units, resolution) {
    ...
    for(var i = 0; i < segments.length; i++) {
        var bottom = point(segments[i][0]);
        var top = point(segments[i][1]);
        ...
        var polyCoords = [[bottomLeft.geometry.coordinates, topLeft.geometry.coordinates]];
        ...
        if(!lineBuffer) {
           lineBuffer = polygon(polyCoords);
        } else {
            lineBuffer = union(lineBuffer, polygon(polyCoords));
        }
    }
    return lineBuffer;
}
